    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setIsOpen(!isOpen)}
                ...
                <div style={{ background: currentTheme?.color }} className="relative w-4 h-4 rounded-full border 
            ...
            <AnimatePresence>
                {isOpen && (<motion.div
                    ...
                    {THEMES.map((t, i) => (<motion.button
                        key={t.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.1 }}
                        className={`relative group w-full flex items-center gap-3 px-3 py-2.5 transition-all duration-200
                        hover:bg-[#262637]  ${theme == t.id ? 'bg-blue-500/10 text-blue-400' : 'text-gray-300'}`}
                        ...
                        {/* color indicator */}
                        <div className="relative size-4 rounded-full border border-gray-600 
                        group-hover:border-gray-500 transition-colors" style={{ background: t.color }} />

                        {/* active theme border */}
                        {theme === t.id && (<motion.div transition={{ type: "spring", bounce: 0.2, duration: 0.6 }}
